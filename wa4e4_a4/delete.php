...
if ( isset($_POST['delete']) && isset($_POST['profile_id']) ) {
    $sqlpro = "DELETE FROM profile WHERE profile_id = :zip";
    $sqlpos = "DELETE FROM position WHERE profile_id = :zip";
    $sqledu = "DELETE FROM education WHERE profile_id = :zip";
    $stmtpro = $pdo->prepare($sqlpro);
    $stmtpos = $pdo->prepare($sqlpos);
    $stmtedu = $pdo->prepare($sqledu);
    $stmtpro->execute(array(':zip' => $_POST['profile_id']));
    $stmtpos->execute(array(':zip' => $_POST['profile_id']));
    $stmtedu->execute(array(':zip' => $_POST['profile_id']));
